...
class _DompetMonthPageState extends State<DompetMonthPage>
  ...
  void initState() {
    super.initState();
    // Get dompetId from DompetBloc if available
    final dompetState = context.read<DompetBloc>().state;
    if (dompetState is DompetLoaded) {
      final dompetId = dompetState.dompet.id;
      // Load transactions filtered by user's dompet
      context.read<TransactionListBloc>().add(
        GetTransactionsByDompetIdEvent(dompetId),
      );
      // Load dompet months for this user's dompet
      context.read<DompetMonthBloc>().add(GetDompetMonthsEvent(dompetId));
    }
              ...
              builder: (context, state) {
                if (state is DompetMonthLoading) {
                  ...
                  if (state.months.isEmpty) {
                    return const Center(child: Text('Belum ada data bulanan'));
                  }
                        ...
                        ElevatedButton(
                          onPressed: () {
                            final dompetState = context
                                ...
                                .state;
                            if (dompetState is DompetLoaded) {
                              context.read<DompetMonthBloc>().add(
                                GetDompetMonthsEvent(dompetState.dompet.id),
                              );
                            }
                          },
              ...
              if (state is TransactionListLoading) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, idx) {
                    ...
                    // Sort transactions within each day by time descending (newest first)
                    final items = grouped[date]!
                      ..sort((a, b) => b.tanggal.compareTo(a.tanggal));

                    // Calculate daily totals
                    double dailyPemasukkan = 0;
                    double dailyPengeluaran = 0;
                    for (final tx in items) {
                      if (tx.type == TypeTransaction.pemasukkan) {
                        dailyPemasukkan += tx.amount;
                      } else {
                        dailyPengeluaran += tx.amount;
                      }
                    }
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    Text(
                                      'Rp ${dailyPemasukkan.toStringAsFixed(0)}',
                                      style: const TextStyle(
                                        fontSize: 12,
                                        color: Colors.green,
                                    ...
                                    const SizedBox(width: 2),
                                    Text(
                                      'Rp ${dailyPengeluaran.toStringAsFixed(0)}',
                                      style: const TextStyle(
                                        fontSize: 12,
                                        color: Colors.red,
                                        fontWeight: FontWeight.w500,
                                          ...
                                          if (id != null) {
                                            ...
                                            if (mounted) {
                                              ...
                                              messenger?.showSnackBar(
                                                const SnackBar(
                                                  content: Text(
                                                    'Tidak dapat menghapus: ID tidak tersedia',
                                                  ),
                                                ),
                                              );
                                            }
